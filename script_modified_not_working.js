...
*/
function lookup(senderID, receiverID){
...
while(1){
    ...
    if(msgArray.length == 14){
        ...
        if(msgArray[0].startsWith("Received")){
            ...
            timeReceived[senderID] = Number(time); //Time is taken in microseconds for
            log.log("Time Received " + timeReceived[senderID] + " Time Sent "+ timeSent[senderID] +"\n");
            //log.log("receiverID " + receiverID + " PacketReceived= " +packetsReceived[receiverID] + "\n");
            //log.log("typeof time " + typeof time + "\n");
            //log.log("typeof timeReceived[senderID] " + typeof timeReceived[senderID] + "\n");
            if(timeReceived[senderID] > 0){
                count[senderID]++;
                //log.log("count[SenderID] "+ count[senderID] +"\n");
                }       
            totalReceived = totalSent = totalCount = totalDelay = 0;
            //totalReceived = totalSent = 0;
            //totalclient=0;
            for(i = 1; i <= nodeCount; i++){
                //totalclient++;
                if(i!=1 && timeReceived[i]>0){
                    delay[i] = timeReceived[i]-timeSent[i];
                    //log.log("delay[i] "+ delay[i] +"\n");
