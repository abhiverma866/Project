...
def process_line(x):
    ...
    try:
        if msgArray2[2] == "P":
            ...
            if msgArray3[1] == "32":
                entry_n32.append([msgArray2[5],msgArray2[6],msgArray2[7],msgArray2[8]])
...
for i in range(1,2):
    ...
    dest_filename = "Jammer" + str(i) + "_energyConsumption.xlsx"
    ...
    for all_cpu, all_lpm, all_tx, all_rx in (entry_n32):
        sheet32.write(row, col, all_cpu)
        sheet32.write(row, col + 1, all_lpm)
        sheet32.write(row, col + 2, all_tx)
        sheet32.write(row, col + 3, all_rx)
        row += 1
